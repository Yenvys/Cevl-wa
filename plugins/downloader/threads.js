...
export default {
  ...
  exec: async (m, { sock, query, command }) => {
    if (!query) {
      return m.reply(
        `_Kirim tautan Threads yang valid!_\n\nContoh: *${m.prefix}${command} https://www.threads.net/...*`,
      );
    }
    if (!/threads\.(net|com)/i.test(query)) {
      return m.reply(
        "_Tautan tidak valid! Pastikan itu adalah tautan dari threads.net atau threads.com._",
      );
    }
    ...
    try {
      ...
      if (!apiKey)
        return m.reply("_JEREXD_API_KEY tidak dikonfigurasi di .env_");
      ...
      if (!data || data.status !== true) {
        await sock.sendMessage(m.from, { react: { text: "❌", key: m.key } });
        return m.reply(
          `_Gagal mengambil media. Error API:* \`\`\`${data?.error || "Respons API tidak valid."}\`\`\``,
        );
      }
      ...
      if (medias.length === 0) {
        await sock.sendMessage(m.from, { react: { text: "❌", key: m.key } });
        return m.reply(
          "_Gagal memuat media. Pastikan tautan berisi video/foto/GIF dan bersifat publik._",
        );
      }

      const authorText = result.author
        ? `*@${result.author.username}*: ${result.author.caption}\n\n`
        : "";
      ...
      if (medias.length > 1) {
        ...
        for (let mediaObj of medias) {
          const isVideo =
            mediaObj.type === "video" || mediaObj.url.includes(".mp4");
          albumMedia.push({
      ...
      if (err.response && err.response.data) {
        ...
        console.error(
          `[THREADS_DL_ERR] API returned ${err.response.status}: ${apiErr}`,
        );
        await m.reply(
          `_Gagal memproses media karena API sedang gangguan._\n\n*Log Error:* \`\`\`${apiErr}\`\`\``,
        );
      } else {
        console.error("[THREADS_DL_ERR]", err.message);
        await m.reply(
          `_Terjadi kesalahan saat menghubungi API:_ ${err.message}`,
        );
      }
    }
  },
