...
async function fetchDownloadUrl(youtubeUrl, targetType = "mp3") {
  try {
    ...
    const res = await axios.get(
      `https://api.jerexd.my.id/api/downloader/youtube?apikey=${apiKey}&url=${encodeURIComponent(youtubeUrl)}&format=${targetType}`,
    );
    const result = res.data?.result;

    if (!result || !result.download)
      throw new Error("Format media tidak ditemukan dari API.");
    ...
    if (!isNaN(index) && index >= 0 && index < session.tracks.length) {
      ...
      try {
        ...
        if (downloadType === "mp3") {
          ...
          exec(
            `"${ffmpegInstaller.path}" -i "${rawFile}" -b:a 128k -map a "${compressedFile}" -y`,
            async (err) => {
              if (err) {
