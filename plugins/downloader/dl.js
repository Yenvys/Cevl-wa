...
async function sendInteractiveMedia(
  sock,
  chatId,
  result,
  sender,
  ...
  try {
    ...
    let imageMessage;
    try {
      const imgResp = await axios.get(thumbnail, {
...
export default {
  ...
  exec: async (m, { sock, query, command }) => {
    if (!query) {
      return m.reply(
        res.format(m.prefix, command, `https://example.com/video`),
      );
    }
    ...
    if (subCommand === "video" && parts[1] !== undefined) {
      ...
      if (!dlCache?.result)
        return m.reply("⚠️ No media found. Please search again.");
      ...
      await sendMediaFile(
        ...
        selected.url,
        "video",
        dlCache.result.title,
        dlCache.result.author,
        dlCache.result.source,
        m,
      );
      await sock.sendMessage(m.from, { react: { text: "", key: m.key } });
      return;
    ...
    if (subCommand === "extractaudio" && parts[1] !== undefined) {
      ...
      try {
        ...
        await sock.sendMessage(m.from, { react: { text: "❌", key: m.key } });
        await m.reply(
          "❌ Failed to extract audio. Try downloading the audio option instead.",
        );
      }
      return;
    }
    ...
    if (subCommand === "photo" && parts[1] !== undefined) {
      ...
      if (parts[1] === "all") {
        ...
        const idx = parseInt(parts[1]);
        if (idx >= photos.length || isNaN(idx))
          return m.reply("❌ Invalid photo selection.");
    ...
    if (!urlMatch)
      return m.reply("❌ Invalid URL. Please provide a valid URL.");
