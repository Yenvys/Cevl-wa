...
export default {
  ...
  exec: async (m, { handler, args, command }) => {
    ...
    if (validModes.includes(targetMode)) {
      ...
      if (targetMode === "group")
        responseText += `> _Bot saat ini hanya akan merespons perintah yang dikirim di dalam obrolan Grup._`;
      if (targetMode === "private")
        responseText += `> _Bot saat ini hanya akan merespons perintah yang dikirim lewat Chat Pribadi (PC)._`;
      if (targetMode === "self")
        responseText += `> _Bot saat ini hanya merespons perintah dari Owner._`;
      if (targetMode === "public")
        responseText += `> _Akses dibuka penuh. Seluruh user bisa menggunakan bot baik di dalam GC maupun PC._`;
      if (targetMode === "groupwl")
        responseText += `> _Bot saat ini hanya merespons di dalam Grup yang terdaftar di Whitelist._`;
      if (targetMode === "userwl")
        responseText += `> _Bot saat ini hanya merespons perintah dari User yang terdaftar di Whitelist._`;

      return m.reply(responseText);
    } else {
      return m.reply(
        `_Opsi yang tersedia hanya: *group*, *private*, *self*, *public*, *groupwl*, atau *userwl*._`,
      );
    }
  },
};
