...
export default {
  ...
  async exec(m, { sock, args, handler, command }) {
    ...
    if (!/image/.test(mime)) {
      return m.reply(
        "Format salah! Harap reply gambar atau kirim gambar dengan .hd/.hd 2_",
      );
    }
    ...
    try {
      ...
      if (!buffer)
        throw new Error(
          "_Gagal mengunduh berkas gambar dari server WhatsApp._",
        );
