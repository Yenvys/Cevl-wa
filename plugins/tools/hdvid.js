...
async function jsonRequest(url, options) {
  ...
  if (!res.ok)
    throw new Error(
      `${options?.method || "GET"} -> ${res.status} ${text.slice(0, 300)}`,
    );
  try {
...
async function enhanceVideoEngine(videoBuffer, resolution = "2k") {
  ...
  if (!jobId)
    throw new Error(`Failed to inject job: ${JSON.stringify(jobRes)}`);

  const done = await pollJob(jobId, headers);
  const outRes = await fetch(done.output_url[0], {
...
export default {
  ...
  desc: "Meningkatkan resolusi kualitas video (AI Video Enhancer).",
  exec: async (m, { sock, args, command }) => {
    ...
    if (!/video/.test(mime)) {
      return sock.sendMessage(
        ...
        {
          text:
            `_Format salah! Kirim video atau reply video dengan caption perintah *${m.prefix}${command}*._\n\n` +
    ...
    try {
      ...
      if (!mediaBuffer) {
        return sock.sendMessage(m.from, {
          text: "_Gagal mengunduh video dari chat, silakan coba kirim ulang video._",
      ...
      if (!result?.buffer)
        throw new Error("Output video kosong atau ditolak oleh server AI.");
