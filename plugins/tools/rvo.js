...
export default {
  ...
  exec: async (m, { sock, command }) => {
    if (!m.quoted)
      return m.reply(
        `_Format salah! reply pada pesan media sekali lihat (*View Once*) menggunakan perintah ${m.prefix}${command}_`,
      );
    ...
    try {
      ...
      if (type === "imageMessage") {
        await sock.sendMessage(
          m.from,
          { image: fs.readFileSync(filePath), caption: msg.caption || "" },
          { quoted: m },
        );
      } else if (type === "videoMessage") {
        await sock.sendMessage(
          m.from,
          { video: fs.readFileSync(filePath), caption: msg.caption || "" },
          { quoted: m },
        );
