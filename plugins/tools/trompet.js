// plugins/downloader/trompet.js
// YouTube Play Trompet Special + SaveTube + FFmpeg Compress + Base64 HTML Music Player
...
/* =========================================================
 * MUSIC PLAYER (TROMPET STYLE)
 * ========================================================= */
function createTrumpetPlayer({ title, artist, duration, audioSrc, imageSrc }) {
  const safeTitle = escapeHtml(title || 'Unknown Trumpet')
  const safeArtist = escapeHtml(artist || 'Trumpet Solo')
...
(function () {
  ...
  if (!audio || !play || !bar || !fill || !dot) return
  ...
  audio.addEventListener('durationchange', function () {
    if (dur && Number.isFinite(audio.duration)) dur.textContent = formatTime(audio.duration)
  })
  ...
  audio.addEventListener('error', function () {
    console.error('[PLAYER] Audio gagal dimainkan')
    setPaused()
  })
  setPaused()
})()
 ...
 * ========================================================= */
export default {
  cmd: ['trompet', 'trumpet', 'playtrompet'],
  category: 'downloader',
  desc: 'Memutar dan mengunduh instrumen trompet dengan player HTML gold',
  exec: async (m, { sock, query, command, text, usedPrefix }) => {
    const conn = sock || m.conn
    let inputQuery = query || text

    // Jika tidak mengisi teks, bot akan otomatis mencari instrumen trompet secara acak
    if (!inputQuery?.trim()) {
      inputQuery = 'trumpet solo instrument'
    } else if (!isYouTubeUrl(inputQuery) && !inputQuery.toLowerCase().includes('trumpet') && !inputQuery.toLowerCase().includes('trompet')) {
      // Otomatis tambahkan kata kunci trompet jika user hanya mengetik nama lagu
      inputQuery = `${inputQuery} trumpet cover`
    }
    ...
    const sendReaction = async (emoji) => {
      ...
      return conn.sendMessage(m.from || m.chat, { react: { text: emoji, key: m.key } })
    }

    await sendReaction("🎺")
    try {
      let input = inputQuery.trim()
      if (!isYouTubeUrl(input)) {
        console.log('[TROMPET] Searching YouTube:', input)
        const search = await yts(input)
        if (!search?.videos?.length) {
          throw new Error('Lagu/Instrumen trompet tidak ditemukan')
        }
        input = search.videos[0].url
      }

      console.log('[TROMPET] Getting YouTube detail...')
      ...
      const title = vid.title || 'Unknown Trumpet'
      const artist = vid.author?.name || 'Trumpet Artist'
      const duration = vid.timestamp || '0:00'

      console.log('[TROMPET] Getting thumbnail...')
      ...
      console.log('[TROMPET] Getting SaveTube audio...')
      ...
      console.log('[TROMPET] Downloading & Compressing...')
      ...
      if (audioSize > MAX_BASE64_AUDIO_SIZE) {
        throw new Error(`Audio Base64 terlalu besar. Maksimal ${MAX_BASE64_AUDIO_MB} MB`)
      }

      console.log('[TROMPET] Creating HTML player...')
      const html = createTrumpetPlayer({
      ...
      console.log('[TROMPET] Sending music player...')
      await sendMusicPlayer(conn, m, html)
      await sendReaction("✅")
    } catch (error) {
      console.error('[TROMPET ERROR]', error)
      await sendReaction("❌")
      ...
      try {
        await conn.sendMessage(
          ...
          {
            text:
              '❌ Gagal memuat instrumen trompet.\n\n' +
              `> ${message}`
          },
          { quoted: m }
        )
      } catch (sendError) {
        console.error('[TROMPET ERROR MESSAGE]', sendError)
      }
    }
  }
}