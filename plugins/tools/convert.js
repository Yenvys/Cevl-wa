...
export default {
  ...
  exec: async (m, { sock, args, command }) => {
    ...
    if (!mime)
      return m.reply(
        "_Format salah! Harap reply berkas media terlebih dahulu sebelum melakukan konversi._",
      );
    ...
    switch (flag) {
      ...
      case "-tomp4":
      case "-tovideo": {
        if (!/video|gif|webp|sticker/.test(mime))
          return m.reply(
            "_Target harus berupa video, gif, atau stiker animasi._",
          );
      ...
      // ==========================================
      case "-togif": {
        if (!/video|webp|sticker/.test(mime))
          return m.reply(
            "_Target harus berupa video atau stiker animasi untuk diubah ke GIF._",
          );
        await sock.sendMessage(m.from, { react: { text: "⏳", key: m.key } });
      ...
      case "-toimage": {
        if (!/webp|sticker/.test(mime))
          return m.reply(
            "_Target harus berupa stiker WebP untuk diubah menjadi gambar._",
          );
        await sock.sendMessage(m.from, { react: { text: "⏳", key: m.key } });
      ...
      case "-toaudio": {
        if (!/video|audio/.test(mime))
          return m.reply(
            "Target harus berupa video atau rekaman suara untuk diubah menjadi audio MP3._",
          );
        await sock.sendMessage(m.from, { react: { text: "⏳", key: m.key } });
