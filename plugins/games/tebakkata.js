...
async function startGame(m, sock, levelName) {
  ...
  if (!words || words.length === 0) {
    return m.reply(
      `❌ Data kata tidak ditemukan atau kosong! Gunakan \`${m.prefix}tk.update\` untuk sinkronisasi.`,
    );
  }
  ...
  exec: async (m, { sock, args, command }) => {
    ...
    if (command === "tk.update") {
      ...
      if (result) {
        ...
        await sock.sendMessage(m.from, { react: { text: "❌", key: m.key } });
        return m.reply(
          "❌ *Sinkronisasi Gagal!* Cek koneksi atau URL sumber data.",
        );
      }
    }
    ...
    if (!levelArg || command.endsWith("?") || levelArg === "?") {
      const helpText = [
        "*♯ TEBAK KATA - CARA BERMAIN*",
        "",
        `Gunakan perintah \`${m.prefix}${command} [level]\` untuk memulai.`,
        "",
        "*Daftar Level & Inisial:*",
        "`e` / `easy`",
        "`m` / `medium`",
        "`h` / `hard`",
        `*♯ Contoh:* \`${m.prefix}${command} m\` atau \`${m.prefix}${command} h\``,
        "",
