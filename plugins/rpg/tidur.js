...
export default {
  ...
  exec: async (m, { sock, command }) => {
    try {
      ...
      if (user.stamina >= 100)
        return m.reply(
          "_Stamina Anda masih dalam kondisi penuh, tidak perlu beristirahat._",
        );

      const cd = getCooldown(user.lastTidur, 60 * 60 * 1000);
      if (!cd.isReady)
        return m.reply(
          `Anda baru saja tidur. Silakan tunggu selama ${Math.ceil(cd.sisaWaktu / 60)} menit lagi untuk beristirahat kembali.`,
        );
      ...
      return m.reply(
        "_Sesi istirahat selesai. Stamina Anda telah sepenuhnya pulih._",
      );
    } catch (e) {
      console.error(e);
