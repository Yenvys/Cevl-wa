  ...
  exec: async (m, { sock, command }) => {
    try {
      ...
      if (user.pekerjaan === "Pengangguran")
        return m.reply(
          "_Anda belum memiliki pekerjaan. Silakan gunakan perintah .apply untuk melamar kerja._",
        );
      ...
      if (new Date() - user.terakhirKerja < timeout) {
        const sisa = Math.ceil(
          (timeout - (new Date() - user.terakhirKerja)) / 1000,
        );
        return m.reply(
          `Mohon tunggu selama *${sisa} detik* lagi untuk menghindari kelelahan.`,
        );
      }

      if (user.stamina < 20)
        return m.reply(
          "_Stamina Anda tidak mencukupi. Silakan gunakan perintah .tidur terlebih dahulu._",
        );

      const jobKey = Object.keys(jobs).find(
        (key) => jobs[key].nama === user.pekerjaan,
      );
      const jobData = jobs[jobKey];
      const task =
        jobData.tasks[Math.floor(Math.random() * jobData.tasks.length)];
      ...
      setTimeout(async () => {
        ...
        if (session && !session.isDone) {
          ...
          await sock.sendMessage(
            m.from,
            {
              text: `🐌 *WAKTU HABIS!*\nAnda terlalu lama merespons. Manajemen menganggap Anda lalai dalam menjalankan tugas.`,
            },
            { quoted: m },
          );
        }
