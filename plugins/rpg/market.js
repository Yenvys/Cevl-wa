  ...
  exec: async (m, { sock, args, command }) => {
    ...
    if (cmdName === "market" || cmdName === "shop") {
      ...
      await sendButton(
        ...
        "*🏮 7NIGHT MARKET 🏮*",
        text,
        "Silakan pilih barang yang ingin dibeli melalui menu di bawah.",
        listButtons,
      );
      return;
    }
    ...
    if (cmdName === "buy") {
      if (!args[0]) {
        return m.reply(
          `Format salah! Gunakan: *${m.prefix}buy <nama_barang>* atau buka *${m.prefix}market*`,
        );
      }
      ...
      if (!item) {
        return m.reply(
          `Barang *"${args[0]}"* tidak ditemukan di pasar! Buka *${m.prefix}market* untuk melihat daftar barang.`,
        );
      }
      ...
      if (user.yen < item.price) {
        return m.reply(
          `Uang Yen Anda tidak cukup untuk membeli *${item.name}*!\nHarga: ${item.price} Yen\nSaldo Anda: ${user.yen} Yen`,
        );
      }
