...
export default {
  ...
  exec: async (m, { args, sock, command }) => {
    try {
      ...
      if (!jobData)
        return m.reply(
          "_Loker fiktif itu! Cek *.bursakerja* untuk loker yang benar!_",
        );

      if (user.statusBermain !== "bebas")
        return m.reply(
          "_Selesaikan dulu urusan/job/interview kamu yang sebelumnya!_",
        );

      if (user.level < jobData.reqLevel)
        return m.reply(
          `Skill/Level mu kurang memadai. level kamu : (Lv.${user.level})! Sedangkan Minimal butuh Lv.${jobData.reqLevel}`,
        );

      if (user.stamina < 30)
        return m.reply(
          "_kamu terlalu lelah, stamina sisa dikit. Pergi *.tidur* dulu untuk mengisi kembali stamina!_",
        );
      ...
      await sendButton(
        sock,
        m.from,
        `📋 *DIVISI INTERVIEW: ${jobData.nama.toUpperCase()}*`,
        body,
        "_Pilih satu opsi jawaban yang benar dari tombol di bawah ini!_",
        listButtons,
      );
    } catch (err) {
      console.error(err);
      ...
      if (choice === jawabanBenar) {
        if (index === 0) {
          ...
          const nextButtons = [
            {
              name: "quick_reply",
              displayText: `A. ${soal2.opsi[0]}`,
              id: "a",
            },
            {
              name: "quick_reply",
              displayText: `B. ${soal2.opsi[1]}`,
              id: "b",
            },
            {
              name: "quick_reply",
              displayText: `C. ${soal2.opsi[2]}`,
              id: "c",
            },
          ];
