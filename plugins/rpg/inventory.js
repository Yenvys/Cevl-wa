...
export default {
  ...
  exec: async (m, { sock, args, command }) => {
    ...
    if (cmdName === "use") {
      if (!args[0]) {
        return m.reply(
          `Format salah! Gunakan: *${m.prefix}use <nama_item>*\nContoh: *${m.prefix}use onigiri*`,
        );
      }
      ...
      if (!item) {
        return m.reply(
          `Barang *"${args[0]}"* tidak bisa digunakan atau tidak ada.`,
        );
      }

      if (!user.inventory[itemKey] || user.inventory[itemKey] <= 0) {
        return m.reply(
          `Kamu tidak memiliki *${item.name}* di inventory! Beli dulu di *${m.prefix}market*.`,
        );
      }

      if (user.stamina >= 100) {
        return m.reply(
          `Staminamu sudah penuh (100/100)! Simpan *${item.name}* mu untuk nanti.`,
        );
      }
