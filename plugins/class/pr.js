...
if (!fs.existsSync(path.dirname(prFile)))
  fs.mkdirSync(path.dirname(prFile), { recursive: true });
...
export default {
  ...
  exec: async (m, { query, command }) => {
      ...
      if (!rawContent) {
        return m.reply(
          res.format(
            m.prefix,
            command,
            `add Matematika | deadline Besok | isi Halaman 40`,
          ),
        );
      }
    ...
    if (action === "list") {
      if (db.length === 0)
        return m.reply("_Tidak ada daftar PR yang tercatat saat ini._");
    ...
    if (action === "del") {
      ...
      if (isNaN(index) || !db[index])
        return m.reply("_Nomor urutan PR yang Anda masukkan tidak valid._");
