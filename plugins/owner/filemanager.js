...
export default {
  ...
  exec: async (m, { sock, args, command }) => {
    ...
    const isSafePath = (target) =>
      target === ROOT_DIR || target.startsWith(ROOT_DIR + sep);
    ...
    try {
      ...
      if (flag === "-mkdir") {
        if (!subArgs.length)
          return m.reply(
            res.format(m.prefix, `${command} -mkdir`, `<nama_folder_baru>`),
          );
        ...
        if (!isSafePath(targetPath))
          return m.reply("_Akses pembuatan ditolak._");
        if (fs.existsSync(targetPath))
          return m.reply("_Direktori folder tersebut sudah ada._");

        await mkdir(targetPath, { recursive: true });
        return m.reply(
          `_Sukses membuat direktori folder baru:_\nPath: \`doc/${relative(ROOT_DIR, targetPath)}\``,
        );
      }
      ...
      if (flag === "-s") {
        ...
        const messageMime =
          (quotedMessage?.msg || quotedMessage)?.mimetype || "";
        if (!messageMime)
          return m.reply(
            "_Format salah! reply dokumen atau berkas media yang ingin disimpan._",
          );
        ...
        const absoluteFileName = newFileNameInput.endsWith(`.${extension}`)
          ? newFileNameInput
          : `${newFileNameInput}.${extension}`;
        const finalFileAbsolutePath = join(targetDirectory, absoluteFileName);

        if (!isSafePath(finalFileAbsolutePath))
          return m.reply("_Akses penyimpanan ditolak._");
      ...
      if (flag === "-cat") {
        ...
        if (!isSafePath(targetPath))
          return m.reply("_Akses pembacaan berkas di luar akar ditolak._");
        if (
        ...
        if (/image/.test(mimeType)) {
          return await sock.sendMessage(
            m.from,
            { image: fileBuffer, caption: basename(targetPath) },
            { quoted: m },
          );
        }
        if (/video/.test(mimeType)) {
          return await sock.sendMessage(
            m.from,
            { video: fileBuffer, caption: basename(targetPath) },
            { quoted: m },
          );
        }
      ...
      if (flag === "-rm") {
        ...
        if (subArgs[0] === "-rf") {
          ...
          if (!folderPath)
            return m.reply(
              "tentukan nama path direktori folder yang ingin dibersihkan.",
            );

          const targetPath = resolve(join(ROOT_DIR, folderPath));
          if (!isSafePath(targetPath) || targetPath === ROOT_DIR)
            return m.reply(res.owner);
          if (!fs.existsSync(targetPath))
            return m.reply("_Direktori tidak ditemukan/Tidak ada._");

          await rm(targetPath, { recursive: true, force: true });
          return m.reply(
            `_Sukses menghapus direktori folder dan isinya:_\nPath: \`doc/${relative(ROOT_DIR, targetPath)}\``,
          );
        }
        ...
        if (!isSafePath(targetPath))
          return m.reply("_Akses modifikasi ditolak._");
        if (!fs.existsSync(targetPath))
          return m.reply("__Direktori yang dituju memang sudah tidak ada._");

        if (fs.lstatSync(targetPath).isDirectory()) {
          return m.reply(
            `_Objek target merupakan direktori folder. Gunakan perintah berikut untuk membersihkannya:_\n\`${m.prefix}${command} -rm -rf ${filePath}\``,
          );
        }

        await unlink(targetPath);
        return m.reply(
          "_Berkas dokumen berhasil dihapus secara permanen dari server server._",
        );
      }

      // Jika flag tidak dikenali
      return m.reply(
        `Flag \`${flag}\` tidak dikenali. Ketik \`${m.prefix}${command}\` untuk melihat daftar flag yang tersedia.`,
      );
    } catch (err) {
      console.error("[FILE_MANAGER_CRASH]", err);
      return m.reply(
        `Terjadi kesalahan pada sistem manajemen berkas internal: ${err.message}`,
      );
    }
  },
};
