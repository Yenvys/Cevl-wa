...
export default {
  ...
  exec: async (m, { sock, command }) => {
    ...
    switch (action) {
      ...
      case "del": {
        if (!input)
          return m.reply(res.format(m.prefix, command, `del <nomor_urut/jid>`));
        let targetJid;
        ...
        return m.reply(
          `_Sukses menghapus user dengan JID: \`${targetJid}\` dari Whitelist._`,
        );
      }
      ...
      case "list": {
        ...
        if (list.length === 0)
          return m.reply(
            "_Tidak ada user yang terdaftar dalam izin Whitelist saat ini._",
          );
