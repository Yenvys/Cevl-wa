  ...
  exec: async (m, { sock, args, command }) => {
    ...
    if (cmdName === "resetdb" || cmdName === "wipedata") {
      ...
      try {
        ...
        return m.reply(
          "_Proses wipe data selesai. Seluruh database pengguna berhasil dikosongkan._",
        );
      } catch (err) {
    ...
    if (opt === "--session" || opt === "-s") {
      const currentSessionId = sock.user?.id
    ...
    if (opt === "--history" || opt === "-h") {
      try {
        ...
        return m.reply(
          "_Seluruh riwayat percakapan AI pada ruang obrolan ini Sukses bersihkan._",
        );
      } catch (e) {
