...
export default {
  ...
  exec: async (m, { sock, args = [], command }) => {
    ...
    if (action === "list") {
      ...
      if (list.length === 0)
        return m.reply("_Tidak ada list nomor Owner yang tercatat._");
      ...
      return await sendMultiContact(sock, m.from, mappedOwners, m);
    }
    ...
    if (action === "add" || action === "remove" || action === "del") {
      ...
      if (!rawJid) {
        return m.reply(
          `_Format salah! Harap tag target, reply pesan target, atau masukkan nomor secara spesifik._\n\nContoh: *${m.prefix}own ${action} @user*`,
        );
      }
      ...
      if (!/^\d{10,15}$/.test(targetNum)) {
        return m.reply(
          "_Format nomor tidak valid! Nomor harus 10-15 digit angka._",
        );
      }
      ...
      if (action === "add") {
        if (config.ownerNumbers.includes(targetNum))
          return m.reply(
            "_Nomor telepon tersebut sudah terdaftar di dalam list Owner._",
          );
        ...
        return m.reply(
          `_Sukses! @${targetNum} saat ini telah resmi ditambahkan sebagai Owner bot tambahan._`,
      ...
      if (action === "remove" || action === "del") {
        ...
        if (targetNum === superOwner)
          return m.reply(
            "_Tidak bisa menghapus super-owner (owner utama) dari daftar._",
          );
        if (!config.ownerNumbers.includes(targetNum))
          return m.reply(
            "_Nomor tersebut memang tidak tercantum di dalam list Owner._",
          );

        config.ownerNumbers = config.ownerNumbers.filter(
          (num) => num !== targetNum,
        );
        saveOwnerConfig(configPath, config.ownerNumbers);

        return m.reply(
          `_Sukses! Otoritas akses Owner untuk @${targetNum} telah resmi dicabut._`,
          { mentions: [targetNum + "@s.whatsapp.net"] },
        );
      }
    }
...
function saveOwnerConfig(filePath, newOwnerList) {
  try {
    ...
    if (regexOwner.test(content)) {
      ...
      console.error(
        "[CONFIG_SAVE_ERR] Properti ownerNumbers tidak ditemukan di config.js",
      );
    }
  } catch (e) {
