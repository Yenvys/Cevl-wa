  ...
  exec: async (m, { sock, command }) => {
    ...
    switch (action) {
      ...
      case "add": {
        ...
        if (!jidAdd.endsWith("@g.us"))
          return m.reply("_Format salah! Input JID harus (@g.us)._");

        const meta = await sock
          .groupMetadata(jidAdd)
          .catch(() => ({ subject: "Grup Tidak Dikenal" }));
        await updateGroupSettings(jidAdd, "whitelist", 1, meta.subject);

        return m.reply(
          `*BERHASIL MENAMBAHKAN WHITELIST*\n\n> *Grup:* ${meta.subject}\n> *JID:* ${jidAdd}`,
        );
      }
      ...
      case "del": {
        if (!input)
          return m.reply(
            "_Format salah! masukkan nomor indeks urutan atau JID grup yang ingin dihapus._",
          );
        let targetJid;
        ...
        return m.reply(
          `_Sukses menghapus grup dengan JID: \`${targetJid}\` dari Whitelist._`,
        );
      }
      ...
      case "list": {
        ...
        if (list.length === 0)
          return m.reply(
            "_Tidak ada grup yang terdaftar dalam izin Whitelist saat ini._",
          );
