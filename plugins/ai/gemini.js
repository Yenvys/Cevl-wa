...
async function runGemini(m, sock) {
  try {
    ...
    const isMedia =
      /image|video|audio|application|webp|pdf|document/i.test(mime) ||
      q.download;

    const quotedIsMedia =
      quoted &&
      (/image|video|audio|application|webp|pdf|document/i.test(
        quoted?.msg?.mimetype || quoted?.msg?.mediaType || "",
      ) ||
        quoted?.download);
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction:
          "Kamu adalah asisten AI yang cerdas. Jawab singkat, natural, tanpa emoji berlebihan.",
...
export default {
  ...
  after: async (m, { sock, handler }) => {
    ...
    const botId = sock.decodeJid
      ? sock.decodeJid(sock.user.id)
      : sock.user.id.split(":")[0] + "@s.whatsapp.net";
    const isFromBot =
      m.quoted.sender === botId ||
      m.quoted.fromMe ||
      (m.quoted.key && m.quoted.key.fromMe);
    ...
    const isFromGemini = history.some((h) => {
      ...
      return (
        m.quoted.text.startsWith(savedText) ||
        savedText.startsWith(m.quoted.text.slice(0, 100))
      );
    });
    ...
    const usedPrefix = handler.prefix.find(
      (p) => p !== "" && m.body?.startsWith(p),
    );
