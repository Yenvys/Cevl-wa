...
export default {
  ...
  exec: async (m, { sock, query, command }) => {
    ...
    if (!/image/.test(mime))
      return m.reply(res.format(m.prefix, command, `[reply/kirim foto]`));
    ...
    try {
      ...
      if (!hasResults) {
        await sock.sendMessage(m.from, { react: { text: "❌", key: m.key } });
        return m.reply(
          "_Pencarian selesai! Tidak ditemukan hasil yang cocok dengan gambar tersebut._",
        );
      }
      ...
      await sock.sendMessage(m.from, { react: { text: "❌", key: m.key } });
      await m.reply(
        "Terjadi kesalahan sistem internal saat memproses data Google Lens: " +
          e.message,
      );
    }
  },
