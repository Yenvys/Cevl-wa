...
class Parser {
  ...
  _getCol(o) {
    return (
      o?.extractedColors?.colorRaw?.hex ||
      o?.extractedColors?.colorDark?.hex ||
      null
    );
  }
  _getVI(v) {
    return v?.squareCoverImage?.extractedColorSet
      ? {
          text_color:
            v.squareCoverImage.extractedColorSet.encoreBaseSetTextColor || null,
          high_contrast:
            v.squareCoverImage.extractedColorSet.highContrast || null,
          higher_contrast:
            v.squareCoverImage.extractedColorSet.higherContrast || null,
          min_contrast:
            v.squareCoverImage.extractedColorSet.minContrast || null,
        }
      : null;
  }
  _getLink(uri) {
    ...
    return {
      uri,
      id: p[2] || null,
      url: p[2] ? `${BASE_URL}/track/${p[2]}` : null,
    };
  }
    ...
    const parse = (arr, mapFn, isTrack = false) =>
      (arr || []).reduce((acc, node) => {
        ...
        if (d)
          acc.push({
            ...mapFn(d),
            ...(node.matchedFields && { matched_fields: node.matchedFields }),
          });
        return acc;
      }, []);
  ...
  async search(query) {
    try {
      if (!(await this.getToken()))
        throw new Error("Gagal generate token internal.");
      const sel = this.cfg.query["search"];
      const { data: res } = await this.is.post(
  ...
  exec: async (m, { sock, query, command }) => {
    if (!query) {
      return sock.sendMessage(
        m.from,
        { text: `_Format salah. Cth: *${m.prefix}${command} lathi*_` },
        { quoted: m },
      );
    }
  ...
  async after(m, { sock }) {
    ...
    try {
      ...
      try {
        ...
        if (targetCover) {
          const imgRes = await axios.get(targetCover, {
      ...
      const msgPayload = {
        ...
        ...(mediaParams?.imageMessage && {
          ...
          thumbnailSha256:
            mediaParams.imageMessage.fileSha256.toString("base64"),
          thumbnailEncSha256:
            mediaParams.imageMessage.fileEncSha256.toString("base64"),
          mediaKey: mediaParams.imageMessage.mediaKey.toString("base64"),
          mediaKeyTimestamp: String(
            mediaParams.imageMessage.mediaKeyTimestamp ||
