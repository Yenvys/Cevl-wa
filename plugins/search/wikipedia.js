...
export default {
  ...
  exec: async (m, { sock, query, command }) => {
    if (!query)
      return m.reply(res.format(m.prefix, command, `<kata_kunci_pencarian>`));
    ...
    try {
      ...
      if (!searchData[1] || searchData[1].length === 0) {
        await sock.sendMessage(m.from, { react: { text: "❌", key: m.key } });
        return m.reply(
          `_Pencarian gagal! Topik mengenai "${query}" tidak ditemukan di Wikipedia._`,
        );
      }
      ...
      await sock.sendMessage(m.from, { react: { text: "❌", key: m.key } });
      await m.reply(
        `_Terjadi kendala teknis saat memproses penarikan data ensiklopedia._`,
      );
    }
  },
};
