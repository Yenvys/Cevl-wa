...
async function resolveDownload(downloadPageUrl) {
  ...
  if (!data || !data.success || !data.data || !data.data.directUrl)
    throw new Error("Failed to resolve download");
  return data.data;
}
        ...
        botForwardedMessage: {
          message: {
            richResponseMessage: {
              messageType: 1,
              unifiedResponse: {
                data: Buffer.from(JSON.stringify(responseData)).toString(
                  "base64",
                ),
              },
              contextInfo: {
...
async function showSearchResults(
  ...
  try {
    let imageMessage;
    try {
      const imgResp = await axios.get(CLOUDINARY_IMAGE, {
...
async function showEpisodeSelection(
  sock,
  chatId,
  seriesTitle,
  episodes,
  ...
  try {
    ...
    if (downloadLinks.length === 0) {
      await sock.sendMessage(chatId, {
        text: "⚠️ No download links available for this title.",
      });
      return true;
    }
    ...
    try {
      if (movieData.poster) {
        const posterUrl = movieData.poster.endsWith(".webp")
          ? movieData.poster.replace(".webp", ".jpg")
          : movieData.poster;
    ...
    await sock.sendMessage(chatId, {
      text: "❌ Failed to show download options.",
    });
    return false;
  }
}
    ...
    try {
      ...
      if (selectIndex !== null) {
        ...
        if (
          ...
          const sent = await showEpisodeSelection(
            sock,
            m.from,
            content.title,
            content.episodes,
          ...
          if (!sent) {
            const epList = content.episodes
              .slice(0, 10)
              .map((ep, i) => `${i + 1}. Episode ${ep.number}`)
              .join("\n");
            await m.reply(
          ...
          const sent = await showDownloadOptions(
            sock,
            m.from,
            content,
            actualQuery,
      ...
      if (downloadIndex !== null) {
        ...
        await m.reply(
          `📤 Sending document (${selectedLink.size})... This may take a while for large files.`,
        );
      ...
      global.db.data.movieSearch[m.sender] = {
        results,
        query: actualQuery,
        timestamp: Date.now(),
      };

      const sent = await showSearchResults(
        sock,
        m.from,
        actualQuery,
        results,
      ...
      if (!sent) {
        const list = results
          ...
          .map(
            (r, i) =>
              `${i + 1}. ${r.title} (${r.year}) - ${r.type} ⭐${r.rating}`,
          )
          .join("\n");
        await m.reply(
