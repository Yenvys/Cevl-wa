...
try {
  ...
  if (integrityResult[0]?.integrity_check !== "ok") {
    console.error(
      "\x1b[1;31m[DB WARN]\x1b[0m Database integrity check gagal:",
      integrityResult,
    );
  }
} catch (e) {
  console.error(
    "\x1b[1;31m[DB ERROR]\x1b[0m Gagal menjalankan integrity check:",
    e.message,
  );
}
...
try {
  db.exec(`ALTER TABLE group_settings ADD COLUMN antilink INTEGER DEFAULT 0;`);
} catch (e) {}
try {
  db.exec(
    `ALTER TABLE group_settings ADD COLUMN auto_close INTEGER DEFAULT 0;`,
  );
} catch (e) {}
try {
  db.exec(`ALTER TABLE contacts ADD COLUMN is_whitelist INTEGER DEFAULT 0;`);
} catch (e) {}

db.exec(
  `CREATE INDEX IF NOT EXISTS idx_whitelist ON group_settings(is_whitelist);`,
);
db.exec(
  `CREATE INDEX IF NOT EXISTS idx_autoclose ON group_settings(auto_close);`,
);
db.exec(
  `CREATE INDEX IF NOT EXISTS idx_contact_whitelist ON contacts(is_whitelist);`,
);
...
export function saveContact(jid, lid, pushName) {
  ...
  const existing = db
    .prepare("SELECT pushname, lid, is_whitelist FROM contacts WHERE jid = ?")
    .get(jid);
  let finalName = pushName;
  if (!pushName || pushName === "Unknown" || pushName === "null") {
    finalName =
      existing && existing.pushname !== "null" ? existing.pushname : "Unknown";
  }
  const currentWhitelist = existing ? existing.is_whitelist : 0;
  db.prepare(
    "INSERT OR REPLACE INTO contacts (jid, lid, pushname, is_whitelist) VALUES (?, ?, ?, ?)",
  ).run(jid, lid || existing?.lid || null, finalName, currentWhitelist);
  if (lid && lid.endsWith("@lid")) {
    db.prepare(
      "INSERT OR REPLACE INTO lid_mapping (lid, jid) VALUES (?, ?)",
    ).run(lid, jid);
  }
}
...
export function isBanned(jid) {
  const row = db
    .prepare("SELECT 1 FROM contacts WHERE jid = ? AND is_whitelist = -1")
    .get(jid);
  return !!row;
}
...
export function incrementGroupMessage(groupJid, userJid) {
  ...
  db.prepare(
    "INSERT INTO group_messages (group_jid, user_jid, count) VALUES (?, ?, 1) ON CONFLICT(group_jid, user_jid) DO UPDATE SET count = count + 1",
...
export function getGroupMessages(groupJid) {
  return db
    .prepare(
      "SELECT user_jid, count FROM group_messages WHERE group_jid = ? ORDER BY count DESC",
    )
    .all(groupJid);
}
...
export function setAfk(jid, groupJid, reason, time) {
  ...
  db.prepare(
    "INSERT OR REPLACE INTO afk (jid, reason, time) VALUES (?, ?, ?)",
  ).run(id, reason, time);
}
...
export function updateGroupSettings(jid, field, value) {
  ...
  if (!col) return;
  db.prepare(`UPDATE group_settings SET ${col} = ? WHERE jid = ?`).run(
    value,
    jid,
  );
}
...
export function syncParticipants(jid, participants = []) {
  ...
  const syncTx = db.transaction((pts) => {
    for (const p of pts) {
      const userJid = p.id?.endsWith("@s.whatsapp.net")
        ? p.id
        : p.phoneNumber
          ? p.phoneNumber.split("@")[0] + "@s.whatsapp.net"
          : null;
...
export function getWhitelistedGroups() {
  return db
    .prepare("SELECT jid, name FROM group_settings WHERE is_whitelist = 1")
    .all();
}

export function getAutoCloseGroups() {
  return db
    .prepare("SELECT jid, name FROM group_settings WHERE auto_close = 1")
    .all();
}
...
export function isUserWhitelisted(jid) {
  if (!jid) return false;
  const row = db
    .prepare("SELECT is_whitelist FROM contacts WHERE jid = ?")
    .get(jid);
...
export function updateUserWhitelist(jid, value, pushname = "Unknown") {
  ...
  if (!existing) {
    db.prepare(
      "INSERT INTO contacts (jid, pushname, is_whitelist) VALUES (?, ?, ?)",
    ).run(jid, pushname, value);
  } else {
    db.prepare("UPDATE contacts SET is_whitelist = ? WHERE jid = ?").run(
      value,
      jid,
    );
  }
...
export function getWhitelistedUsers() {
  return db
    .prepare("SELECT jid, pushname FROM contacts WHERE is_whitelist = 1")
    .all();
}
