...
export const sendButton = async (
  ...
  try {
    ...
    const msgId = sock.generateMessageID
      ? sock.generateMessageID()
      : "YENVY_BTN_" + Math.random().toString(36).substring(7).toUpperCase();
    ...
    try {
      const fallbackLines = buttons.map((btn) => {
        if (btn.name === "quick_reply")
          return `• *${btn.displayText}* -> ketik: ${btn.id}`;
        if (btn.name === "cta_url")
          return `• *${btn.displayText}* -> Link: ${btn.url}`;
        if (btn.name === "cta_copy")
          return `• *${btn.displayText}* -> Salin: ${btn.id}`;
