...
export async function startSock(
  ...
  if (!sock.authState.creds.registered) {
    ...
    userPilihan = await question(
      chalk.yellow.bold(" Masukkan pilihan (1/2): "),
    );
    ...
    if (userPilihan === "1") {
      ...
      if (!phoneNumber) {
        phoneNumber = await question(
          chalk.bgMagenta.white.bold(" Masukan Nomer Bot (Contoh: 628xxx): "),
        );
      }
      ...
      const requestPairing = async (retries = 0) => {
        try {
          ...
          if (retries < 5) {
            ...
            logger.error(
              "PAIRING",
              "Gagal request pairing code setelah 5 percobaan.",
            );
          }
        }
      };
    ...
    sock.ev.on("connection.update", async (update) => {
      ...
      if (connection === "close") {
        ...
        if (statusCode === DisconnectReason.loggedOut) {
          ...
          setTimeout(() => {
            startSock(sessionId, logger, onSocketReady, _retryCount + 1)
              .then(resolve)
              .catch(reject);
          }, delay);
        }
