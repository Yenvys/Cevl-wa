...
console.log = (...args) => {
  const isSessionLog = args.some((arg) => {
    ...
    if (typeof arg === "object" && arg !== null) {
      return (
        "_chains" in arg || "registrationId" in arg || "currentRatchet" in arg
      );
    }
    return false;
  });
  ...
  if (isSessionLog) {
    ...
    return originLog(
      `${time} ${tag} ${label} \x1b[90mEncrypted session bundle ignored.\x1b[0m`,
    );
  }
...
export class Logger {
  ...
  universal(m) {
    ...
    if (m.from === "status@broadcast") {
      ...
      header = this.color(
        94,
        `CHANNEL: ${m.groupName || "Unknown"} (${m.from})`,
      );
    } else if (m.isGroup) {
    ...
    const typeMsg =
      typeMap[m.type] || (m?.type ? m.type.replace("Message", "") : "unknown");
    ...
    console.log(
      `           ${this.color(90, "╰──〉")}${pushName} : ${msgTypeLabel} ${content}`,
    );
  }
  ...
  cmd(m, plugin) {
    ...
    console.log(
      `${time} ${tag} ${this.color(93, "[EXEC]")} ${this.color(37, m.prefix + cmdName)} by ${this.color(33, m.pushName)}`,
    );
  }

  info(...args) {
    console.log(
      this.color(90, `[${this.getTime()}]`),
      this.color(35, `<${this.prefix}>`),
      this.color(96, "[INFO]"),
      ...args,
    );
  }
  ...
  res(m, text) {
    ...
    const snippet =
      typeof text === "string"
        ...
        : "Media/Object";
    console.log(
      `${time} ${tag} ${this.color(94, "[RESP]")} ⮕  "${snippet}..."`,
    );
  }
}
