  ...
  try {
    ...
    if (tmpFiles.length > 0) {
      for (const file of tmpFiles) {
        try {
          fs.unlinkSync(path.join(customTemp, file));
        } catch {}
      }
      console.log(
        `[STARTUP] Cleaned ${tmpFiles.length} temp file(s) from data/tmp/`,
      );
    }
  } catch {}
}
...
for (const { key, required, label } of envValidation) {
  if (!process.env[key]) {
    if (required) {
      console.error(
        `\x1b[1;31m[ENV ERROR]\x1b[0m ${label} (${key}) tidak ditemukan! Beberapa fitur tidak akan bekerja.`,
      );
    } else {
      console.warn(
        `\x1b[1;33m[ENV WARN]\x1b[0m ${label} (${key}) kosong — fitur terkait akan dinonaktifkan.`,
      );
    }
  }
}
...
process.on("uncaughtException", (err) => {
  ...
  try {
    store.writeToFileSync(storeFilePath);
  } catch {}
  process.exit(1);
});
